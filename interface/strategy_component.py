...
class StrategyEditor(tk.Frame):
    def __init__(self,root, binance: BinanceFuturesClient, bitmex: BitmexFuturesClient, *args, **kwargs):
        ...
        self._base_params = [
            {'code_name': 'strategy_type', 'widget': tk.OptionMenu, 'data_type':str,'values' :['Technical', 'Breakout'], 'width': 10,'header':'Strategy'},
            {'code_name': 'contract', 'widget': tk.OptionMenu, 'data_type': str, 'values' : self._all_contracts, 'width': 15,'header':'Contract'},
            {'code_name': 'timeframe', 'widget': tk.OptionMenu, 'data_type': str, 'values': self._all_timeframes,'width': 10,'header':'Timeframe'},
            ...
            {'code_name': 'parameters', 'widget': tk.Button, 'data_type': float, 'text': 'Parameters', 'bg': BG_COLOR_2, 'command': self._show_popup,'header':'', 'width': 10},
            {'code_name': 'activation', 'widget': tk.Button, 'data_type': float, 'text': 'OFF', 'bg': 'darkred', 'command': self._switch_strategy,'header':'', 'width': 8},
            {'code_name': 'delete', 'widget': tk.Button, 'data_type': float, 'text': 'X', 'bg': 'darkred', 'command': self._delete_row,'header':'', 'width': 6}
        ]
    ...
    def _add_strategy_row(self):
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                self.body_widgets[code_name][b_index] = tk.Entry(self._body_frame.sub_frame,bd=1, justify=tk.CENTER,font=GLOBAL_FONT, width=base_param['width'])
                ...
                self.body_widgets[code_name][b_index] = tk.Button(self._body_frame.sub_frame, text = base_param['text'], bg=base_param['bg'],width=base_param['width'], fg=FG_COLOR, command=lambda frozen_command= base_param['command']: frozen_command(b_index))
            else:
                continue
            self.body_widgets[code_name][b_index].grid(row=b_index, column=col,padx=3)

        self.additional_params[b_index] = dict()
    ...
    def _show_popup(self, b_index: int):
        ...
        for param in self.extra_params[strat_selected]:
            ...
            if param['widget'] == tk.Entry:
                ...
                if self.additional_params[b_index][code_name] is not None:
                    self._extra_input[code_name].insert(tk.END, str(self.additional_params[b_index][code_name]))
            else:
                continue
    ...
    def _validate_parameters(self, b_index:int):
        ...
        for param in self.extra_params[strat_selected]:
            ...
            if self._extra_input[code_name].get() == '':
                ...
                self.additional_params[b_index][code_name] = param['data_type'](self._extra_input[code_name].get())
    ...
    def _switch_strategy(self, b_index: int):
        ...
        if self.body_widgets['activation'][b_index].cget("text") == 'OFF':
            ...
            if strat_selected == 'Technical':
                new_strategy = TechnicalStrategy(self._exchanges[exchange], contract, exchange, timeframe, balance_pct, take_profit, stop_loss, self.additional_params[b_index])

            elif strat_selected == 'Breakout':
                new_strategy = BreakoutStrategy(self._exchanges[exchange], contract, exchange, timeframe, balance_pct, take_profit, stop_loss, self.additional_params[b_index])
    ...
    def _load_workspace(self):
        ...
        for row in data:
            ...
            extra_params = json.loads(row['extra_param'])

            for param, value in extra_params.items():
                if value is not None:
                    self.additional_params[b_index][param] = value
